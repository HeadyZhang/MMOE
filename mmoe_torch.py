...
class MMOE(nn.Module):
    ...
    def forward(self, x):
        ...
        _x_rep = torch.reshape(x_rep, (dims[0], dims[1] * dims[2]))  ## (batch_size, sparse_x_len * emb_size)

        experts_o = [e(_x_rep) for e in self.experts]
        experts_o_tensor = torch.stack(experts_o)

        gates_o = [self.softmax(_x_rep @ g) for g in self.w_gates]
    ...
    ## 商聊卡片
    def cal_loss(self, is_impr_list, is_pic_list, outputs):
        criterion = nn.BCELoss(reduction='mean')
        loss1 = criterion((outputs.T)[0], is_impr_list)
        loss2 = criterion((outputs.T)[1], is_pic_list)
        loss = loss1 * 0.4 + loss2 * 0.6
        return loss 
    
    def cal_loss_v2(self, is_eff_rev_list, is_eff_pic_list, outputs):
        criterion = nn.BCELoss(reduction='mean')
        loss1 = criterion((outputs.T)[0], is_eff_rev_list)
        loss2 = criterion((outputs.T)[1], is_eff_pic_list)
